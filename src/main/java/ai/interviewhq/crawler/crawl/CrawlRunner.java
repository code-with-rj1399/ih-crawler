...
public class CrawlRunner {
    ...
    public void scheduledRun() {
        if (!settings.getBoolean(CrawlerSettings.CRON_ENABLED, true)) {
            log.info("Scheduled crawler run skipped: disabled by admin configuration");
            return;
        }
        runOnce("scheduled");
    }
    ...
    public synchronized void runOnce(String trigger) {
        ...
        log.info("=== crawler run started: jobId={}, trigger={}, lookbackHours={} ===",job.getId(),trigger,job.getLookbackHours());
        ...
        try {
            ...
            for(CrawlSource source:sources) futures.add(executor.submit(()->crawlSource(job.getId(),source)));
            ...
            job.setSourcesOk(ok);job.setSourcesFailed(failed);job.setPagesFetched(pages);job.setPagesSkipped(skipped);job.setPostsExtracted(posts);job.setQuestionsUpserted(questions);job.setBlockedCount(blocked);job.setFinishedAt(Instant.now());
            job.setStatus(failed==0?CrawlJob.SUCCEEDED:(ok==0?CrawlJob.FAILED:CrawlJob.PARTIAL));
            jobRepository.save(job);
        } catch(Exception e){job.setStatus(CrawlJob.FAILED);job.setErrorSummary(e.getMessage());job.setFinishedAt(Instant.now());jobRepository.save(job);throw e;
        } finally {executor.shutdown();log.info("=== crawler run finished: jobId={} ===",job.getId());}
    }
    ...
    private void crawlSource(Integer jobId,CrawlSource source){
        ...
        try {
            ...
            sub.setFinishedAt(Instant.now());sub.setPagesFetched(processed[0]);sub.setPagesSkipped(skipped[0]);sub.setPostsExtracted(savedPosts[0]);sub.setQuestionsUpserted(savedQuestions[0]);sub.setModelCalls(modelCalls[0]);sub.setBlockedCount(blocked[0]);subRunRepository.save(sub);
        }
    }
    ...
    private void processEntry(CrawlSource source,ParsedEntry entry,Instant cutoff,int cap,int[] processed,int[] savedPosts,int[] savedQuestions,int[] skipped,int[] modelCalls,Set<String> seenHashes){
        if(processed[0]>=cap){skipped[0]++;return;} processed[0]++;
        ...
        try{
            TwoStepOpenAiExtractor.ExtractionResult result=extractor.extract(source,postUrl,entry.title(),entry.author(),entry.publishedAt(),entry.bodyText());
            ExperienceExtraction experience=result.experience();modelCalls[0]+=experience!=null&&!experience.questions().isEmpty()?2:1;if(experience==null){skipped[0]++;return;}

...
public class LeetcodeDiscussAdapter implements SourceAdapter {
    ...
    public List<ParsedEntry> crawl(CrawlSource source, Instant lookback, PoliteFetcher fetcher) throws Exception {
        ...
        return crawlGraphql(source, lookback, fetcher, category);
    }
    ...
    private List<ParsedEntry> crawlGraphql(CrawlSource source, Instant lookback, PoliteFetcher fetcher, String category)
        ...
        if (!result.robotsAllowed()) {
            throw new IllegalStateException("LeetCode GraphQL request blocked by robots policy");
        }
        if (!result.isSuccess()) {
            throw new IllegalStateException("LeetCode GraphQL returned HTTP " + result.status());
        }

        JsonNode root = mapper.readTree(result.bodyAsString());
        if (root.has("errors")) {
            log.warn("LeetCode GraphQL errors: {}", root.get("errors"));
            throw new IllegalStateException("LeetCode GraphQL returned errors");
        }

        JsonNode edges = root.path("data").path("categoryTopicList").path("edges");
        if (!edges.isArray()) {
            throw new IllegalStateException("LeetCode GraphQL response missing categoryTopicList.edges");
        }
        ...
        log.info("LeetCode GraphQL returned {} entries for category={}", entries.size(), category);
        return entries;
    }

...
public class TwoStepOpenAiExtractor {
    ...
    public ExtractionResult extract(CrawlSource source, String postUrl, String title, String author,
        ...
        try {
            ...
            // Step 2 is page-scoped: all questions extracted from this page are sent in one model call.
    ...
    private ExperienceExtraction extractExperience(CrawlSource source, String postUrl, String title,
        ...
        return new ExperienceExtraction(firstNonBlank(extractedTitle, title), summary, postedAt, extractedAuthor,
                company, role, level, location, candidateYoE, outcome, rounds, candidates, authentic);
    }
    ...
    private JsonNode experienceSchema() {
        ...
        root.set("properties", props); root.set("required", required("authenticExperience", "experience", "questions"));
    ...
    public record ExtractionResult(ExperienceExtraction experience, List<InterviewQuestion> questions) {
        public ExtractionResult { questions = questions == null ? List.of() : List.copyOf(questions); }
        public static ExtractionResult empty() { return new ExtractionResult(ExperienceExtraction.empty(), List.of()); }
    }

    public record QuestionMetadata(String questionText, boolean isValidInterviewQuestion, String questionType,
                                   String difficulty, List<String> topics, String questionDescription, Float confidence) {
        public QuestionMetadata { topics = topics == null ? List.of() : List.copyOf(topics); }
        public static QuestionMetadata empty() { return new QuestionMetadata(null, false, null, null, List.of(), null, 0f); }
    }
}

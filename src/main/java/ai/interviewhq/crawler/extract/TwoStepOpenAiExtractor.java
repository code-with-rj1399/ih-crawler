...
public class TwoStepOpenAiExtractor {
    ...
    public ExtractionResult extract(CrawlSource source, String postUrl, String title, String author,
        ...
        if (source == null || postUrl == null || postUrl.isBlank() || bodyText == null || bodyText.isBlank()) return ExtractionResult.empty();
        if (apiKey == null || apiKey.isBlank()) { log.warn("OPENAI_API_KEY missing - skipping extraction for {}", postUrl); return ExtractionResult.empty(); }
        try {
            ExperienceExtraction experience = extractExperience(source, postUrl, title, author, publishedAt, bodyText);
            if (experience.questions().isEmpty()) return new ExtractionResult(experience, List.of());
            ...
            for (int i = 0; i < experience.questions().size(); i++) {
                if (i >= metadata.size()) {
                    log.warn("Step 2 returned fewer results for {}: candidates={}, metadata={}", postUrl, experience.questions().size(), metadata.size());
                    continue;
                }
        ...
        } catch (Exception e) { throw new IllegalStateException("Two-step interview extraction failed for " + postUrl, e); }
    }

    private ExperienceExtraction extractExperience(CrawlSource source, String postUrl, String title, String author, Instant publishedAt, String bodyText) throws Exception {
        String prompt = experiencePrompt.replace("{{source_platform}}", safe(source.getName())).replace("{{post_url}}", safe(postUrl)).replace("{{title}}", safe(title)).replace("{{author}}", safe(author)).replace("{{published_at}}", publishedAt == null ? "" : publishedAt.toString()).replace("{{page_content}}", truncate(bodyText));
        ...
        String extractedTitle = nullableText(experience, "title"); String summary = nullableText(experience, "summary");
        Instant postedAt = parseInstant(nullableText(experience, "postedAt")); if (postedAt == null) postedAt = publishedAt;
        String extractedAuthor = firstNonBlank(nullableText(experience, "author"), author); String company = nullableText(experience, "company");
        String role = nullableText(experience, "role"); String level = nullableText(experience, "level"); String location = nullableText(experience, "location");
        Float candidateYoE = nullableFloat(experience, "candidateYoE"); List<QuestionCandidate> candidates = new ArrayList<>(); JsonNode questions = result.path("questions");
        if (questions.isArray()) for (JsonNode question : questions) { String text = question.isTextual() ? question.asText() : nullableText(question, "questionText"); if (text != null && !text.isBlank()) candidates.add(new QuestionCandidate(text.trim())); }
        return new ExperienceExtraction(firstNonBlank(extractedTitle, title), summary, postedAt, extractedAuthor, company, role, level, location, candidateYoE, candidates);
    }

    private List<QuestionMetadata> extractQuestionMetadata(List<QuestionCandidate> candidates) throws Exception {
        ArrayNode questions = objectMapper.createArrayNode(); for (QuestionCandidate candidate : candidates) questions.add(candidate.questionText().trim());
    ...
    private InterviewQuestion toQuestion(QuestionMetadata metadata) {
        ...
        question.setQuestionGranularity(metadata.questionGranularity());
        question.setConfidence(metadata.confidence());
        question.setModelName(settings.extractModel());
        question.setExtractedAt(Instant.now());
        question.setDedupeHash(Hashing.questionDedupeHash(null, question.getQuestionText()));
        return question;
    }

    private JsonNode callOpenAi(String prompt, JsonNode schema, String schemaName) throws Exception {
        ObjectNode request = objectMapper.createObjectNode(); request.put("model", settings.extractModel()); request.put("input", prompt); request.put("max_output_tokens", settings.extractMaxTokens()); request.set("tools", objectMapper.createArrayNode()); request.put("store", false);
        ObjectNode reasoning = objectMapper.createObjectNode(); reasoning.put("effort", "low"); request.set("reasoning", reasoning); ObjectNode text = objectMapper.createObjectNode(); text.set("format", schema.path("format")); text.put("verbosity", "low"); request.set("text", text);
        HttpRequest httpRequest = HttpRequest.newBuilder(RESPONSES_URI).timeout(Duration.ofSeconds(180)).header("Authorization", "Bearer " + apiKey).header("Content-Type", "application/json").POST(HttpRequest.BodyPublishers.ofString(request.toString())).build();
    ...
    private JsonNode parseOutput(JsonNode root) throws Exception {
        if (root == null) return null; String outputText = root.path("output_text").asText(null); if (outputText != null && !outputText.isBlank()) return objectMapper.readTree(cleanJson(outputText));
        JsonNode output = root.path("output"); if (!output.isArray()) return null;
        for (JsonNode item : output) { if (!"message".equals(item.path("type").asText())) continue; JsonNode content = item.path("content"); if (!content.isArray()) continue; for (JsonNode contentItem : content) { if (!"output_text".equals(contentItem.path("type").asText())) continue; String text = contentItem.path("text").asText(null); if (text != null && !text.isBlank()) return objectMapper.readTree(cleanJson(text)); } } return null;
    }

    private JsonNode experienceSchema() {
        ObjectNode format = jsonSchemaFormat("experience_extraction"); ObjectNode root = objectSchema(); ObjectNode props = objectMapper.createObjectNode(); ObjectNode experience = objectSchema(); ObjectNode ep = objectMapper.createObjectNode();
        ep.set("title", nullableStringSchema()); ep.set("summary", nullableStringSchema()); ep.set("postedAt", nullableStringSchema()); ep.set("author", nullableStringSchema()); ep.set("company", nullableStringSchema()); ep.set("role", nullableStringSchema()); ep.set("level", nullableStringSchema()); ep.set("location", nullableStringSchema()); ep.set("candidateYoE", nullableNumberSchema());
        experience.set("properties", ep); experience.set("required", required("title", "summary", "postedAt", "author", "company", "role", "level", "location", "candidateYoE")); props.set("experience", experience); props.set("questions", stringArraySchema()); root.set("properties", props); root.set("required", required("experience", "questions")); format.set("schema", root); return objectMapper.createObjectNode().set("format", format);
    }

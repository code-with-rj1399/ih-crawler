...
public class DynamoDbRepositorySupport {
    ...
    public void ensureTable() {
        try {
            ...
            for (int i = 0; i < 30; i++) {
                try {
                    if (TableStatus.ACTIVE.equals(client.describeTable(DescribeTableRequest.builder().tableName(tableName).build()).table().tableStatus())) return;
    ...
    public <T> T save(T entity, String pk, String sk) {
        ...
        item.put("pk", AttributeValue.builder().s(pk).build()); item.put("sk", AttributeValue.builder().s(sk).build()); item.put("entityType", AttributeValue.builder().s(entity.getClass().getSimpleName()).build()); item.put("data", toAttributeValue(data));
        client.putItem(PutItemRequest.builder().tableName(tableName).item(item).build()); return entity;
    }
    ...
    public <T> Optional<T> find(Class<T> type, String pk, String sk) {
        ...
        if (!response.hasItem()) return Optional.empty(); return Optional.of(fromItem(type, response.item()));
    }
    ...
    public int deleteAllByEntityType(Class<?> type) {
        String entityType = type.getSimpleName(); int deleted = 0; Map<String, AttributeValue> start = null;
        do {
            ScanRequest.Builder request = ScanRequest.builder().tableName(tableName).filterExpression("entityType = :entityType").expressionAttributeValues(Map.of(":entityType", AttributeValue.builder().s(entityType).build())); if (start != null && !start.isEmpty()) request.exclusiveStartKey(start);
            var response = client.scan(request.build()); for (var item : response.items()) { AttributeValue pk = item.get("pk"); AttributeValue sk = item.get("sk"); if (pk != null && sk != null) { delete(pk.s(), sk.s()); deleted++; } } start = response.lastEvaluatedKey();
        } while (start != null && !start.isEmpty()); return deleted;
    }
    ...
    public String hashKey(String value) {
        try { byte[] digest = MessageDigest.getInstance("SHA-256").digest(Objects.requireNonNullElse(value, "").getBytes(StandardCharsets.UTF_8)); StringBuilder hex = new StringBuilder(digest.length * 2); for (byte b : digest) hex.append(String.format("%02x", b)); return hex.toString(); }
    ...
    private Object fromAttributeValue(AttributeValue value) {
        if (value == null) return null; if (value.s() != null) return value.s(); if (value.n() != null) { String n = value.n(); try { return n.contains(".") ? Double.parseDouble(n) : Long.parseLong(n); } catch (NumberFormatException ignored) { return n; } }
        if (value.bool() != null) return value.bool(); if (value.nul() != null && value.nul()) return null; if (value.m() != null) { Map<String, Object> result = new HashMap<>(); value.m().forEach((k, v) -> result.put(k, fromAttributeValue(v))); return result; }
